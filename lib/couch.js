...
couch.prototype.saveUserGoogleToken = function (identity, token, callback) {

	if (!identity) return callback(new Error ('No user identity supplied to save google token for.'))
	if (!token) return callback(new Error ('No token supplied to save for user.'))
